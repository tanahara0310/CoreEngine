...
> **現況（2026-07-29 / Stage 3 完了時点）**: RTシャドウ 3 パスは **0.347ms（フレームの約 5.8%）**。
> 上の 1〜4 章は調査当時（Stage 0 以前）の記録として残してある。
> 最新の数値と実装は 5 章の「Stage 3」を参照。
...
### Stage 3 — リソース管理を伴う影の最適化 ✅ 実装済み（2026-07-29）

**検証ゲート結果: 目標 1.0ms に対して 0.347ms。大幅に超過達成。**

#### 実測（同一セッション・同一構図で Stage 2 を測り直した A/B）

前セッションの数値と混ぜないため、**Stage 3 の変更を `git stash` で退避して
Stage 2 のバイナリを同一セッションで測り直した**。その結果は前セッションの値を
小数第 4 位まで再現したので、両者は直接比較できる。

| ステージ | Stage 2 | Stage 3<br>(フル解像度) | Stage 3<br>(ハーフ解像度・既定) |
|---|---|---|---|
| RTShadowTrace | 1.1397 ms | 0.5714 ms | **0.1626 ms** |
| RTShadowTemporal | 0.2263 ms | 0.0963 ms | **0.0430 ms** |
| RTShadowDenoise | 0.3901 ms | 0.1946 ms | **0.1413 ms** |
| **合計** | **1.7561 ms** | **0.8623 ms (-51%)** | **0.3469 ms (-80%)** |

（1920×991 / Sun 1 灯 / 保存済み DebugCamera 構図 / `rayDist(実効)=1743.4`）

Stage 1 開始時点の 2.409ms からは **-86%**。フレーム比 22.8% → **約 5.8%**。

#### 【重要な発見】Trace の半減は「DXIL ライブラリの未参照リソース宣言」だった

フル解像度のまま Trace が 1.14 → 0.57ms（**-50%**）になった。切り分け結果:

| 実験 | RTShadowTrace | 結論 |
|---|---|---|
| Stage 3 / R8_UNORM / フル解像度 | 0.5701 ms | — |
| Stage 3 / **R32_FLOAT** / フル解像度 | 0.5714 ms | **フォーマットは Trace に無関係**（帯域は律速でない） |
| Stage 3 / R32 / フル + **未使用 SRV テーブル 2 本をルートシグネチャへ復活** | 0.5745 ms | **ルートシグネチャのサイズも無関係** |

残る差分は **`RTShadow.hlsl` から未参照の `gHistoryShadow`(t3) と
`gMotionVector`(t4) の宣言を削除したこと**だけ。
テンポラル蓄積を別パスへ切り出した時点でこの 2 つは RayGen から読まれなくなっていたが、
宣言だけが残っていた。

> **`lib_6_6`（DXIL ライブラリ）は未参照リソースを削除しない。**
> ライブラリはリンク時のインターフェースを保つ必要があるため、
> 通常のシェーダーなら DXC が落とす宣言がそのまま残り、
> RayGen シェーダーのリソースフットプリント＝オキュパンシーを押し下げる。
> レイトラバースはオキュパンシー律速なので、これがそのまま 2 倍の差になった。
>
> **教訓: DXR シェーダーでは「使っていないリソース宣言」は死コードではなくコストである。**
> 水面 RT 3 本（`RTWaterCaustics/Reflection/Refraction.hlsl`）は全宣言が参照済みで
> この問題は無いことを確認済み。
...
| 項目 | やったこと |
|---|---|
| 履歴 ping-pong | `HistoryA` / `HistoryB` の 2 枚を毎フレーム入れ替え。テンポラルパスは前フレーム履歴を読んで今フレーム履歴へ UAV 書き込みするだけになり、**全画面 `CopyResource`（約 16MB/frame）が消えた** |
| A-Trous のパリティ制約撤廃 | ping/pong 専用の `DenoiseA` / `DenoiseB` を用意。最終パスの書き込み先だけ切り替えるので**パス数 0〜4 の任意値**が指定できる（Stage 2 までは偶数のみ） |
| `R8_UNORM` 化 | マスクは 0〜1 の 1ch なので R32_FLOAT は過剰。メモリは 1/4（1920×991 で 25MB/灯 → 約 3MB/灯）。**ただし Trace への効果はゼロ**（上表）。効くのはデノイズ/テンポラル側 |
| ハーフ解像度トレース | トレース〜テンポラル〜A-Trous を 1/2 解像度で回し、最後にバイラテラルアップサンプルでフル解像度マスクへ解決。サンプル位置は 2x2 をフレームごとに巡回させ、テンポラル蓄積で 4 サブピクセルぶんを回収する |
| 解決パス | `RTShadowResolve.CS.hlsl` を新規追加。フル解像度の深度・法線と、各トレーステクセルが代表するピクセルの深度・法線を比べて重み付けする。幾何一致サンプルが無いときは空間重みのみの平均へフォールバック（1.0 へ逃がすと輪郭に白い縁が出る） |
| 死コード削除 | RayGen の未使用 `gHistoryShadow` / `gMotionVector` / `gHistoryAlpha` を削除（上記のとおり実コストだった）。`RayTracingOutputViewSet::kMaxViewCount` の旧名エイリアスを撤去 |
| 共通化 | デノイズ / テンポラル / 解決の 3 本が「SRV テーブル n 個 + UAV 1 個 + ルート定数 1 個」という同じ形だったので `CreateComputePass()` に集約（約 120 行の重複を削除） |

#### スロット構成（`RayTracingOutputViewSet` の平坦スロット）

```
(view, light) あたり 6 枚 → 2 view × 4 light × 6 = 48 スロット（上限 64）
  Mask      … フル解像度。DeferredLighting が読む実体
  Raw       … トレース解像度。RayGen の生出力（A-Trous に潰されないよう専用スロット）
  DenoiseA  … トレース解像度。A-Trous ping
  DenoiseB  … トレース解像度。A-Trous pong
  HistoryA  … トレース解像度。テンポラル履歴（偶数フレーム）
  HistoryB  … トレース解像度。テンポラル履歴（奇数フレーム）
```

フル解像度モード（`halfResolutionTrace = false`）では A-Trous の最終パスが
`Mask` へ直接書くため、解決パスは走らない（等倍コピーの無駄を避ける）。

#### 検証

- 静止画: 椰子の幹・葉の影の輪郭が保たれ、ハーフ解像度由来のブロックノイズ・
  R8_UNORM 由来のバンディング・アップサンプル由来の白縁のいずれも無し
- カメラ回転中（右ドラッグ 30 ステップ中にキャプチャ）: ゴースト・尾引き・黒いチカつき無し
- Debug / Development / Release の 3 構成すべて警告ゼロでビルド通過

#### 意図的に見送った項目とその理由

- **裏面早期棄却（`dot(N,L) <= 0`）**: 効果は Trace の約 9%＝**0.015ms** まで縮んだ一方、
  マスクは IBL・空・環境光・コースティクスの遮蔽にも流用されている
  （`DeferredLighting.PS.hlsl:534-640`）ため、裏面に 1.0 を書くと環境光が変わる。
  正しくやるには直接光マスクと環境光マスクの分離＝DeferredLighting の意味づけ変更が要る。
  **得られる 0.015ms に対して見合わない。**
- **影専用 LOD1 BLAS**: `SubMeshData::lods[]` に LOD1/LOD2 の範囲は既にあるが、
  `BLASDesc` が「インデックスバッファ 1 本」前提でマルチジオメトリ BLAS を作れない。
  この整備は Stage 4（BLAS ライフサイクル API）の一部であり、
  Trace が 0.16ms まで落ちた今、先行して入れる理由が無い。

#### 空いた予算の使い道

`softShadowSamples` は今も既定 1（＝1 ピクセル 1 本）。
コストはほぼレイ本数に比例するので、**2spp にしても +0.16ms 程度**の見込みで、
それでも Stage 2 の 1/3 で収まる。当初の要望だった
「ちゃんとしたソフトシャドウ」はデバッグパネルのスライダで即座に試せる。
