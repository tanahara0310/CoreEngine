...
> **既定値の唯一の情報源は `Engine/Src/Graphics/Water/WaterFoamDefaults.h`。**
> 泡の値は CVar → `WaterFrameConstants` → `FFTOceanManager::FoamSettings` → `FoamConstants`
> と 4 段で運ばれるが、各段の構造体は自前のリテラルを持たずこのヘッダーを参照する。
> 値を変えるときはヘッダーだけを直すこと（下表は根拠の記録であって定義ではない）。
>
> 以前は各段が独立にリテラルを持っており、`r.Water.Foam.Bias` の既定値だけが
> 旧 Water.json 由来の 0.806 のまま残って他 3 か所（0.85）と食い違っていた
> （`DecaySeconds` も 3.07 と 3.0 で二重化）。実行時は CVar が毎フレーム上書きするため
> 描画には出ず、「較正値はどっちなのか」がコードから判断できない形でだけ現れる事故だった。
...
| cascadeWeight | 1.0 / 0.5 / 0.2 | 勾配は波数 k に比例するため無重みだと最小カスケード(31m)が支配し detJ が広範囲で飽和する（Phase 0 実測で確定した較正値。設計当初の 1.0/1.0/0.5 では足りなかった） |
