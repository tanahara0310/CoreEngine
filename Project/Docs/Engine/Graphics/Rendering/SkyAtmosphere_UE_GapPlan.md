...
- [ ] 昼: 影の中がわずかに青い
- [ ] 夕方: 影の中がオレンジ〜紫に変化する
- [ ] 日没後: アンビエントがほぼゼロ（Phase 1 と合わせてシーン全体が夜になる）
- [ ] キューブマップ空シーンは従来アンビエントのまま
...
- [ ] 天頂・昼の見た目がほぼ変わらない（積分式変更の回帰確認。微差は許容）
- [ ] 夕暮れの地平線グラデーションが滑らかになり、反太陽側の地平線際が暗くなる
- [ ] MultiScatteringFactor=0 で空が単一散乱のみ（特に薄明時に顕著に暗くなる）ことを確認
- [ ] 雲・ゴッドレイ・レンズフレアに回帰が無い（同じ LUT を参照しているため必ず確認）
