...
### 検証記録（2026-07-14: 空アンビエントの効果実測と「反太陽側が太陽側と同じ明るさ」問題の診断）

**方法**: ①シェーダー積分（AtmosphereCommon.hlsli / MultiScatteringLUT.CS / SkyViewLUT.CS /
SkyIrradianceSH.CS）を C++ へ忠実移植した CPU レプリカで HDR 放射輝度を直接計測。
②実機（AtmosphereTestScene）で ImGui 経由で太陽高度 1.9°に設定し、方位角 0↔180°の切替で
同一画面領域の表示輝度（sRGB 逆変換後のリニア Y）を比較。

**① 空アンビエント（SH9）で何が変わったか** — トグル OFF（旧ハーフランバート）↔ ON の実測:

| 状況 | 対象 | OFF (旧) | ON (SH) | 変化 |
|---|---|---|---|---|
| 昼 30° | 地面 | RGB(229,227,221) 白飛び気味 | RGB(153,162,176) | 約2倍暗く・空由来の青みに |
| 昼 30° | キューブ | RGB(219,215,219) | RGB(166,179,204) | 同上 |
| 薄暮 1.9°反太陽側 | 地面 | RGB(136,55,3) 強いオレンジ | RGB(29,28,36) | **6.7倍暗い青灰**（直接光ほぼゼロなので物理的に正しい） |

旧アンビエントは「ライト色×強度×ハーフランバート」でライトが届かない時間帯でも
地表を照らし続けていた。SH 版は空の実輝度に追従するため、**差が最も現れるのは薄暮〜夜**。
昼は skyAmbientScale=0.3 が旧実装と概ね揃うよう校正されているため差に気づきにくい（意図どおり）。

**② 反太陽側の明るさは正しいか** — 結論: **HDR 放射輝度は物理的に妥当。
「同じ明るさに見える」のは露出（表示変換）の問題**。

- CPU レプリカ実測（太陽高度 +2°、地平線際 el 2°）: 太陽側 Y=1.49 / 反太陽側 Y=0.32 →
  **比 4.7倍**。CIE Clear Sky (Type 12) の散乱指標関数から求まる比（約5倍）と整合。
  太陽が沈むと地球影で比が拡大: -2°で 20.6倍 / -4°で 254倍（Belt of Venus 側が消灯）。
- 実機の表示輝度（トーンマップ後）: EV0 固定 → 比 **1.7倍**（ACES の肩で両者とも飽和域に
  乗り潰れる）。自動露出 ON → 比 1.4倍（視線ごとに正規化されるため差はさらに縮む。
  ただし現実の眼・カメラも同じ挙動であり誤りではない）。**EV −2 → 比 2.6〜2.9倍**
  （太陽側だけ明帯が残る現実的な見え方になる）。
- 多重散乱（等方 Psi_ms）の反太陽側寄与は太陽高度 +2°で Y の約17%。既知の過大評価傾向は
  あるが支配的ではない（multiScatteringFactor で調整可能）。
- Sky-View LUT のサンプラーは CLAMP 済み（反太陽端 u=1 → 太陽端 u=0 への WRAP 巻き込み無し）。

**推奨**: 大気側のパラメータはいじらない（物理的に正しい範囲）。夕暮れの見た目を現実に
近づけたい場合は露出で対応する — 自動露出（Krawczyk 自動キー・実装済み）を有効にするか、
薄暮シーンで手動 EV −2 前後を使う。
...
**診断記録（2026-07-15）: 「反太陽側が太陽側と同じ明るさに見える」は方位バグではない。**
HLSL パイプライン（Transmittance/MS/Sky-View LUT ＋ PS サンプリング）を C# へ忠実移植した
オフラインシミュレーションで、太陽高度 2° のとき太陽側/反太陽側の放射輝度比は
視線高度 1° で 6.7 倍・太陽高度 0.5° では 8.8 倍と、正しい方位非対称を生成していることを確認
（LUT 経由と直接積分は誤差 1% 未満で一致＝パラメータ化・解像度にも問題なし）。
実機スクリーンショットの行スキャンでも太陽方向への輝度勾配（画面端 151 → 太陽付近 237）を確認。
**同じ明るさに見える原因は `sunIntensity=20` × ACES の圧縮域**で、物理輝度差 4〜9 倍が
表示上 1.4〜1.9 倍まで潰れるため（＋雲による遮蔽）。自動露出 ON なら知覚どおりになるが、
既定は引き続きオプトインとする（2026-07-15 ユーザー判断・現状維持）。
...
**✅ 実装済み（2026-07-15）** — 距離スケールの CB 化（`apKmPerSlice`・エディタ「空気遠近感 距離スケール」
スライダー 1〜16 km/slice）と、半透明（水面）への AP 適用（新規 `AtmosphereApply.hlsli`・
`ApplyAerialPerspective()`）、および 128km 超の Sky-View フォールバック。
`AtmosphereConstants` は 208 バイトのまま（`constantsPad` の Vector3 → `float apKmPerSlice + float[2]`）。

**配管検証の記録（2026-07-15）**: 見た目の変化が微小で判別困難だったため、3 段階の切り分けで配管を実証した。
①水面 PS 末尾で `return float3(10,0,0)`（赤）→ 水面が赤く塗られ、**フラグ到達とバインドを確認**。
②距離を `×1.0`（メートルを km 扱い＝1000 倍誇張）→ カメラを引くと水面が距離依存で変化することを確認
（＝CameraVolume LUT・SkyView フォールバック・CB が全て接続済み）。③正しい `×0.001` へ戻して本番確認。
**本番スケールでは水面は数十〜数百 m 先しかなく AP はごく僅か**（正しい挙動。UE でも近距離の水面 AP は微小）。
`×1000` 診断で「カメラを引くと暗くなる」現象が出たが、これは誇張により水面の下向き視線が 128km 超の
**Sky-View フォールバック領域（下向きは LUT の地面側＝暗い値を引く）**へ押し込まれたための副作用で、
実スケールでは発生しない（水面は 128km に届かず、フォールバックは共有ヘルパーの安全網として残す）。
...
- [x] 水面の遠方が地形・不透明物と同じように大気で霞む（本番スケールでは微小。配管は上記 3 段階診断で実証）
- [~] `apKmPerSlice` を変えると霞の距離感が変わり、LUT 再生成が正しく走る（CB 経路・`paramsChanged`→`MarkLUTDirty` は実装済み。
      本番スケールでは水面での視覚差が小さく目視確認は未実施。地形シーンでの目視は今後）
- [x] AP 非対応シーン・大気非アクティブシーンで半透明の見た目が変わらない
      （`gAerialPerspectiveEnabled` フラグを SkyBox の大気モード＋LUT/CB 準備完了で立てる。
      キューブマップ空シーンでは false のまま＝従来の水面描画）
- [x] 回帰なし（WaterTestScene の水面・反射・屈折が正常、ログにエラー 0、Water.PS コンパイル成功）
