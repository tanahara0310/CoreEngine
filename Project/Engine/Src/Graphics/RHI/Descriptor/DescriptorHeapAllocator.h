...
{
    /// @brief ディスクリプタヒープ 1 本ぶんのスロット確保器
    ///
    /// @details
    /// SRV/CBV/UAV・RTV・DSV は「シェーダから見えるか」と「1 スロットのバイト数」が違うだけで、
    /// 確保・解放・ハンドル計算のロジックは同一。旧 DescriptorAllocator はこれを 3 つ分
    /// コピペしていた（確保／解放／ハンドル計算／mutex／フリーリスト／二重解放検出 ×3）。
    /// このクラス 1 つを 3 インスタンス持つことで重複を無くす。
    ///
    /// @note 確保・解放はスレッドセーフ（ワーカースレッドからのテクスチャロードが SRV を確保する）。
