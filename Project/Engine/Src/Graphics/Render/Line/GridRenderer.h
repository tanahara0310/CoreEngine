...
/// @details カメラ高度で最小格子間隔を決め、10 倍ずつ粗い段を 3 つ重ねてクロスフェードする。
///          線はカメラ中心の円で切って縁を α で落とすため、実質無限グリッドに見える。
/// @note GameObject ではなく ILineSource。GridFeature が所有し、Line パスのフラッシュ時に供給する。
