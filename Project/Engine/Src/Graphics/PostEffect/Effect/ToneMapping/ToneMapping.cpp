...
namespace CoreEngine
{
    void ToneMapping::Initialize(DirectXCommon* dxCommon)
    {
        ...
        compiler.Initialize();
        computeShaderBlob_ = compiler.CompileShader(L"ToneMapping.CS.hlsl", L"cs_6_0");
        ...
        if (!buildResult.success) {
            throw std::runtime_error("ToneMapping: Failed to create RootSignature: " + buildResult.errorMessage);
        }
    ...
    void ToneMapping::CreateComputePipeline()
    {
        ...
        if (FAILED(hr)) {
            throw std::runtime_error("ToneMapping: Failed to create Compute PSO");
        }
    }

    void ToneMapping::CreateConstantBuffer()
    {
