...
namespace CoreEngine
{
    ...
    public:
        ...
        /// @brief 全ロガーの保留分をディスクへ書き出す（完了まで待つ）
        /// @details ロガーは非同期なので、キューに積んだだけの行はプロセスが
        ///          異常終了すると丸ごと消える。落ちる直前に必ず呼ぶこと。
        ///          flush_on(err) は「ワーカーがそのメッセージを処理したとき」に
        ///          効くだけで、処理される前に死ねば意味が無い。
        void Flush();
