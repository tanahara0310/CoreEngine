...
namespace CoreEngine
{
    ...
    ///
    /// @details **このクラスの存在意義は「回帰検知」であって「原因究明」ではない。**
    ///          スコープを置いた場所しか見えないので、どこが遅いか見当も付かない状態の
    ///          調査には向かない。そこは VS のパフォーマンスプロファイラ（Alt+F2）や
    ///          Windows Performance Recorder（`wpr.exe`）を使うこと。
    ///          こちらの強みは「毎起動タダで出る」「ステップ名という
    ///          ドメインの語彙で出る」「ログに残るので差分で回帰を追える」の 3 点。
    ///
    /// @details **壁時計と CPU 時間を必ず並べて出すのが設計の要。**
    ///          2026-08-18 に「あるステップが 2.2 秒かかっている」を
    ///          そのステップのコストだと誤読して、無意味なリファクタリングに
    ///          着手しかけた。実際はメインスレッドの実作業は 31ms で、残りは
    ///          非同期プリロードのワーカーとの競合による**待ち**だった。
    ///          `壁時計 ≫ CPU 時間` が見えていれば一発で分かった。
    ///
    /// @warning **CPU 時間（`GetThreadTimes`）の分解能は約 15.6ms（スケジューラの刻み）。**
    ///          数十 ms 未満のスコープで待ち率を読んではいけない。
    ///          このクラスが答えられるのは「数百 ms 単位の区間で待っているか働いているか」まで。
    class CpuProfiler {
    public:
        /// @brief 1 スコープの計測結果
        struct Sample {
