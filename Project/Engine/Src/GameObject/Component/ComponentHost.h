...
/// @brief コンポーネントを保持する器
///
/// @details `GameObject` がこれを継承する。**参照の安定性**と**解放の遅延**という
///          2 つの性質を持たせてあり、どちらも当たり判定リファクタリング（Phase 3〜6、
///          PASS 29 / FAIL 0）で `ColliderComponent` が実証済みのパターンをそのまま一般化したもの。
///
///          **参照の安定性**: 各コンポーネントは個別にヒープへ確保する。したがって
///          コンポーネントを追加しても、既に配布済みの `T*` は無効化されない。
///          `AddComponent()` がポインタを返す API を安全に保つための要件。
///
///          **解放の遅延**: `RemoveComponent()` は実体を即 delete せず `retired_` へ退避する。
///          衝突コールバックや Update の中から着脱されても、判定ループや呼び出し元が
///          保持している生ポインタが宙に浮かない。実体の解放はフレーム末の
///          `ReleaseRetiredComponents()`（`GameObjectManager` が衝突判定より後に呼ぶ）で行う。
///
///          **イテレーション中の着脱**: 取り外しは配列から要素を消さず **スロットを
///          nullptr にする**（インデックスをずらさない）。詰め直しは
///          `ReleaseRetiredComponents()` がフレーム末に行う。
class ComponentHost {
public:
