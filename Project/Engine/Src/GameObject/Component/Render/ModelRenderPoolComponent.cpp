...
COMPONENT_REGISTER(CoreEngine::ModelRenderPoolComponent)
...
namespace CoreEngine
{
    ...
    json ModelRenderPoolComponent::OnSerialize() const {
        json result = {
        ...
        if (!modelPath_.empty()) {
            result["model"] = AssetPathToJson(modelPath_);
        }
        if (color_) {
    ...
    void ModelRenderPoolComponent::OnDeserialize(const json& j) {
        // 要素を作る前だけ、保存データのモデルを使う
        if (entries_.empty() && j.contains("model")) {
            modelPath_ = JsonToAssetPath(j.at("model"), "ModelRenderPool の model");
        }
...
#ifdef USE_IMGUI
    bool ModelRenderPoolComponent::DrawInspector() {
        bool changed = false;
    ...
    void ModelRenderPoolComponent::Awake() {
        if (modelPath_.empty()) {
    ...
    void ModelRenderPoolComponent::Update() {
    ...
    void ModelRenderPoolComponent::OnDestroy() {
        for (Entry& entry : entries_) {
            if (entry.object && !entry.object->IsMarkedForDestroy()) {
    ...
    bool ModelRenderPoolComponent::Draw(
    ...
    std::size_t ModelRenderPoolComponent::GetActiveCount() const {
        const std::uint64_t frame = Time::FrameCount();
    ...
    ModelRenderPoolComponent::Entry* ModelRenderPoolComponent::CreateEntry() {
        GameObject* owner = GetOwner();
        if (!owner || modelPath_.empty()) {
            return nullptr;
        }
    ...
    void ModelRenderPoolComponent::ApplyEntryColor(
    ...
    void ModelRenderPoolComponent::BeginFrameIfNeeded(std::uint64_t frame) {
    ...
    ModelRenderPoolComponent::Entry* ModelRenderPoolComponent::FindEntryForPosition(
    ...
    ModelRenderPoolComponent::Entry* ModelRenderPoolComponent::FindAvailableEntry(std::uint64_t frame) {
    ...
    void ModelRenderPoolComponent::ResizePool(std::size_t capacity) {
    ...
    void ModelRenderPoolComponent::ApplyColorToEntries() {
        for (Entry& entry : entries_) {
