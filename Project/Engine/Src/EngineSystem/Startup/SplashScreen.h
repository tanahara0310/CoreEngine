...
namespace CoreEngine
{
    ...
    ///
    /// @details D3D12 デバイスができる前から表示する必要があるので、あえて GDI で描く。
    ///          スワップチェーンもシェーダも要らないぶん、起動の一番最初
    ///          （デバイス作成より前）から画が出せるのが利点。
    ///
    ///          メインウィンドウは起動シーケンスが終わるまで非表示のままにし、
    ///          この小さなウィンドウだけがメッセージを処理する。これで
    ///          「全画面の白い無反応ウィンドウ」が出る時間がゼロになる。
    class SplashScreen {
    public:
