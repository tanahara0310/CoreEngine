...
namespace CoreEngine
{
	void ModelRenderer::Initialize(ID3D12Device* device) {
		...
		// Root Parameter 12: PBR NormalMap用ディスクリプタテーブル (t7, PS)
		...
		// Root Parameter 13: PBR MetallicMap用ディスクリプタテーブル (t8, PS)
		...
		// Root Parameter 14: PBR RoughnessMap用ディスクリプタテーブル (t9, PS)
	...
	// Root Parameter 15: PBR AOMap用ディスクリプタテーブル (t10, PS)
	...
	// Root Parameter 16: IBL IrradianceMap用ディスクリプタテーブル (t11, PS)
	...
	// Root Parameter 17: IBL PrefilteredMap用ディスクリプタテーブル (t12, PS)
	...
	// Root Parameter 18: IBL BRDF LUT用ディスクリプタテーブル (t13, PS)
	...
	if (irradianceMapHandle_.ptr != 0) {
		cmdList->SetGraphicsRootDescriptorTable(ModelRendererRootParam::kIrradianceMap, irradianceMapHandle_);
	}

	// IBL Prefiltered Mapをセット（未設定の場合はスキップ）
	if (prefilteredMapHandle_.ptr != 0) {
		cmdList->SetGraphicsRootDescriptorTable(ModelRendererRootParam::kPrefilteredMap, prefilteredMapHandle_);
	}

	// BRDF LUTをセット（未設定の場合はスキップ）
	if (brdfLUTHandle_.ptr != 0) {
		cmdList->SetGraphicsRootDescriptorTable(ModelRendererRootParam::kBRDFLUT, brdfLUTHandle_);
	}
}
