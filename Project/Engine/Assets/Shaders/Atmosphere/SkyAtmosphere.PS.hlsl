...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    if (intersectGround && viewDir.y < -1e-5f)
    {
        ...
        if (tGround > 0.0f)
        {
            ...
            // 直接光 + 常夜灯的な最小アンビエント（夜間も完全な黒穴に戻らないようにする）
            const float kGroundLightScale = 5.0f;   // 美術値。実床（DeferredLighting）の明るさに大まかに合わせる
            const float kGroundAmbientFloor = 0.03f;
            float3 groundColor = gAtmosphere.groundAlbedo * checker * gAtmosphere.sunColor
                                * (NdotL * sunTransAtGround * kGroundLightScale + kGroundAmbientFloor);
