...
void WaterSurfaceParameterPanel::Initialize(WaterSurfaceRuntimeController& runtimeController, EngineSystem& engine) {
	...
	if (auto* renderDomainContext = engine.GetRenderDomainContext()) {
		if (auto* waterRefractionManager = renderDomainContext->GetWaterRefractionRayTracingManager()) {
			rtRefractionOffsetPixels_ = waterRefractionManager->GetSettings().maxRefractionOffsetPixels;
		}
	}
...
void WaterSurfaceParameterPanel::Draw(WaterSurfaceRuntimeController& runtimeController, EngineSystem& engine) {
...
void WaterSurfaceParameterPanel::ApplyRayTracingSettings(EngineSystem& engine) const {
	// UI で変更した DXR 屈折設定をレイトレーシング管理側へ反映する
	if (auto* renderDomainContext = engine.GetRenderDomainContext()) {
		if (auto* waterRefractionManager = renderDomainContext->GetWaterRefractionRayTracingManager()) {
...
void WaterSurfaceParameterPanel::DrawRuntimeParameterSection(
	...
	if (ImGui::SliderFloat("DXR屈折ずれ量 (px)", &rtRefractionOffsetPixels_, 0.0f, 12.0f, "%.2f")) {
		ApplyRayTracingSettings(engine);
	}
